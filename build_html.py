...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* 大小球分组样式 */
  th.group-ou-base { background: #b52b2b; }   /* 基础盘口 */
  th.group-ou-hv1 { background: #8b1a1a; }   /* HV1 */
  th.group-ou-hv2 { background: #6b1414; }   /* HV2 */
  /* 添加右侧分隔线，区分三组 */
...
function doSearch() {
  ...
  var theadHTML =
    ...
    '  <tr>' +
    '    <th rowspan="2">时间戳</th>' +
    '    <th colspan="12" class="group-ng">进球分布赔率</th>' +
    '    <th colspan="8" class="group-tg">总进球概率</th>' +
    '    <th colspan="9" class="group-ou-base">大小球赔率 + 公平概率 + EV</th>' +
    '  </tr>' +
  ...
  // 大小球子标题，三组分别添加 class 和分隔线（最后一列不带分隔线）
  theadHTML +=
    '    <th class="group-ou-base">大球OO</th>' +
    '    <th class="group-ou-base">小球UO</th>' +
    '    <th class="group-ou-base ou-divider">盘口LI</th>' +
    '    <th class="group-ou-hv1">HV1大</th>' +
    '    <th class="group-ou-hv1">HV1小</th>' +
    '    <th class="group-ou-hv1 ou-divider">HV1盘</th>' +
    '    <th class="group-ou-hv2">HV2大</th>' +
    '    <th class="group-ou-hv2">HV2小</th>' +
    '    <th class="group-ou-hv2">HV2盘</th>' +
    '  </tr></thead>';
  ...
  for (var ri = 0; ri < rows.length; ri++) {
    ...
    for (var lix = 0; lix < 3; lix++) {
      ...
      // 为每组最后一列添加分隔线 class（HV1盘和HV2盘之后不加，HV2盘是最后一列无需分隔）
      var divClassO = (lix === 0) ? ' ou-divider' : '';  // 基础组最后一列加分隔线
      var divClassU = (lix === 1) ? ' ou-divider' : '';  // HV1组最后一列加分隔线

      if (isEmpty) {
        htmlOut += '<td class="ou-val"><span class="missing">-</span></td>';
        htmlOut += '<td class="ou-val"><span class="missing">-</span></td>';
        htmlOut += '<td class="ou-val' + divClassO + '"><span class="missing">-</span></td>';
      } else {
        // 大球 cell (赔率/公平P/EV)
        var cellO = ovStr;
        ...
        // 盘口 cell（添加分隔线）
        htmlOut += '<td class="ou-val' + chH + divClassO + '">' + hcStr + '</td>';
      }
    }
